...
`homebridge-ratgdo` will publish MQTT events if you've configured a broker in the plugin settings. It exposes the garage door, light, lock, motion, obstruction, and occupancy state, along with the device-specific sensors and switches on Ratgdo (ESP32) Disco and Konnected hardware. The topics tables below are the complete, authoritative list.
