...
To get started with `homebridge-ratgdo`:
  ...
  * If your device has the ESPHome native API encryption key enabled, set that key in the plugin's feature options (the `Device.Encryption.Key` option for that device) so the plugin can connect to it. Devices without API encryption need no key.
